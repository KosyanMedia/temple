  ...
  module.exports = function(templates_files){
    ...
    function builder(instructions) {//Single-pass translator
      ...
      for(var i = 0, l = instructions.length; i < l; i++) {
        ...
        if(ins[0] == 'attr') {
          if(!declarated_attributes[ins[2] + i]){
            declarated_attributes[ins[2] + i] = true;
            declarations.push('var ' + ins[2] + i + '_attr = document.createAttribute("' + ins[2] + '");');
          }
          if(ins[3][0] == 'V') {
            var variable = split_var(ins[3][1]);
            declarations.push('var ' + variable[0] + '_attr = document.createTextNode("");');
            links.push(ins[2] + i + '_attr.appendChild(' + variable[0] + '_attr)');
            ...
            links.push(ins[2] + i + '_attr.appendChild(document.createTextNode("' + esc(ins[3][1]) + '"))');
          }
        } else if(ins[0] == 'text') {
          if(ins[2][0] == 'V') {
